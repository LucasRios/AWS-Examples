...
=================================
Receives a list of EC2 instance IDs via the event payload and starts them.
...
No credentials are stored in this function.
The SDK automatically uses the IAM role attached to the Lambda execution environment.
"""
...
def lambda_handler(event, context):
    ...
    dict
        ...
        which instances were started and their new state transitions.
    """

    # --- 1. Validate input ---------------------------------------------------

    # Extract the list of instance IDs from the event.
    # The caller must provide this key; an empty list is treated as a no-op.
    instance_ids = event.get("instance_ids", [])

    if not instance_ids:
        # Nothing to do — return early with a clear message instead of failing.
    ...
    print(f"Starting instances: {instance_ids} in region {region}")

    try:
        response = ec2.start_instances(InstanceIds=instance_ids)
    except ec2.exceptions.ClientError as e:
        # Covers cases like invalid instance IDs, permission errors, etc.
        error_message = str(e)
        print(f"ERROR calling start_instances: {error_message}")
        return _response(500, {"message": "Failed to start instances.", "error": error_message})

    # --- 5. Build result summary ---------------------------------------------

    # The response from start_instances contains a list of StateChange objects,
    ...
    return _response(200, {
        "message": f"Start command sent to {len(state_changes)} instance(s).",
